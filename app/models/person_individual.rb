...
class PersonIndividual < ActiveRecord::Base
  has_one :person, as: :capacity, dependent: :destroy
  accepts_nested_attributes_for :person
  
  validates :name, :surname, :gender, :cpf, :birthdate, presence: true
  validates :name,  length: { maximum: 30 }, 
                    format: { with: /\A[a-zA-Z]+\z/ },
                    message: 'Somente primeiro nome'
  validates :surname, length: { maximum: 70 }
